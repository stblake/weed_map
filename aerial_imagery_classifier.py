...
def imagery_statistical_binary_classification(image_bgr, samples_A_bgr, samples_B_bgr, 
    ...
    if imagery_id is not None:
        id_str = os.path.basename(imagery_id).replace('.JPG','')
    else:
        id_str = 'UNKNOWN'

    if plotting:
        show_bgr_image(image_bgr, title = f'{id_str} - original image')

    # Histogram equalisation. 
    image_bgr = histogram_equalise(image_bgr)
...
try decreasing \'duplicate_tolerance\', which is currently {duplicate_tolerance}.')
    ...
    # for i,n_sigma in enumerate(n_sigma_thresholds,1):
    #     spray_region[density < mean - n_sigma*std] = 1
    ...
    return density, classification_image, spray_region
